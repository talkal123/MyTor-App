...
const BusinessById = () => {
  ...
  const withoutYear = dateString.split(" ").slice(0, 3).join(" ");
    ...
    useEffect(() => {
  if (id) {
    axios.get(`http://localhost:3000/business/${id}`)
      .then((response) => {
    ...
    useEffect(() => {
  if (id) {
    axios.get(`http://localhost:3000/appointment/business/${id}`)
      .then((response) => {
...
const filteredAppointments = businessAppointment
          ...
          <div className='flex flex-col justify-between h-full'>
            <div className=''>
              ...
              <div className=' border-b-1 pt-5 pb-5 flex gap-5 items-center'>
                <div>
                  <img src={businessData?.images?.[1]} className='w-20 h-20 rounded-lg' alt="" />
                </div>
                <div className='flex flex-col gap-2'>
                  <div>
                    <h3 className='font-bold text-lg'>{businessData?.services?.map((item) => item.name)}</h3>
                  </div>
                  <div className='flex gap-1'>
                    <h3 className='font-semibold'>${businessData?.services?.map((item) => item.price)}</h3>
                    <span className='text-center'>•</span>
                    <span className='text-gray-300'>{businessData?.services?.map((item) => item.serviceTime)} minutes</span>
                  </div>
                </div>
              </div>
              <div className='pt-5 flex flex-col gap-8'>
                ...
                <div >
                  <input onChange={(e) => setTime(e.target.value)} type="date" name="" id="" value={time} />
                </div>
              </div>
              <div className='flex flex-wrap gap-2 p-5 pt-10 pb-10'>
  ...
  <div className="flex flex-wrap gap-2">
    {filteredAppointments
      ...
      .sort((a, b) => a.time.localeCompare(b.time))
      .map((item, index) => (
        <div key={index} className='border w-20 h-20 rounded-lg flex items-center justify-center'>
          <span>{item.time}</span>
        </div>
      ))
    }
